...
async def login_user(user: LoginUser, request: Request):
    ...
    if "contact" in user_data and user_data["contact"]:
        try:
            ...
            print(mobile)
            message = f"Login Alert: Successful login at {login_time} from IP {ip_address}. Device: {user_agent}"
            
            # Prepare mobile number (remove leading + if present)
            # mobile_number = mobile[1:] if mobile.startswith("+") else mobile
...
class HealthInput(BaseModel):
    ...
    bp_systolic: float
    bp_diastolic: float
    cholesterol: float  # Added new required field
...
@app.post("/predict-health-issue", response_model=PredictionOutput)
async def predict_health_issue(input_data: HealthInput):
    try:
        # Load the trained model
        rf_model = joblib.load("random_forest_helth_issuev2.joblib")
        
        # Create input DataFrame with the exact feature names used during training
        input_df = pd.DataFrame([[input_data.age, input_data.gender, 
                               input_data.family_history, input_data.bp_systolic,
                               input_data.bp_diastolic, input_data.cholesterol]],
                             columns=['Age', 'Gender', 'Family History', 
        ...
        return {
            "prediction": prediction,
            "disease_label": DISEASE_MAPPING.get(prediction, "Unknown"),
            "error": None
        }
...
# Chatbot settings
MODEL_PATH = os.path.abspath("chatbot_model/unsloth.Q4_K_M.gguf")
...
def clean_response(text: str) -> str:
    banned_phrases = [
        "http://", "https://", "www.", "contact our customer care",
        "call", "reach us at", "+1", "doctor.com"
    ]
...
async def chat(request: ChatRequest):
    system_prompt = (
        ...
        "Do not suggest medicine, drugs, or locations.\n"
        "Do not provide links, phone numbers, or refer to specific services.\n"
        "Your goal is to help users feel better by offering encouragement, empathy, and healthy lifestyle tips.\n"
        "If a user expresses stress, anxiety, or sadness, respond with compassion and emotional support.\n"
    ...
    safe_text = clean_response(text)

    return {"response": safe_text if safe_text else "Error: No output from AI"}
